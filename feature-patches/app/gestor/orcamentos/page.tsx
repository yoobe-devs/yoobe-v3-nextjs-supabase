...
import { BudgetWizard } from '@/components/ui/BudgetWizard'
...
export default function GestorOrcamentosPage() {
  ...
  // New wizard and management states
  const [viewMode, setViewMode] = useState<'list' | 'wizard' | 'management'>('list')
  const [selectedBudget, setSelectedBudget] = useState<Budget | null>(null)
  const [wizardMode, setWizardMode] = useState<'create' | 'edit' | 'view'>('create')
  // Removido modal redundante - usar apenas galeria

  // Modal antigo removido — usando galeria para criação
  ...
  useEffect(() => {
    ...
    // Timeout de segurança para forçar o carregamento
  ...
  const loadData = async () => {
    try {
      ...
      // Carregar orçamentos usando API de desenvolvimento
      try {
        const budgetsResponse = await fetch('/api/gestor/budgets/dev')
      ...
      // Carregar produtos de forma simplificada
      try {
        const productsResponse = await supabase
          ...
          .order('name')
          .limit(50) // Limitar para evitar travamento
      ...
      // Carregar status de replicação de forma assíncrona (não bloqueia)
      setTimeout(async () => {
        try {
          const arr = budgets
          ...
          // Carregar apenas os primeiros 5 orçamentos para evitar travamento
  ...
  // saveBudget removido — criação via galeria abaixo
  ...
  // preview removido — não utilizado no novo fluxo

  // manipuladores de itens do modal removidos
  ...
  // New wizard and management functions
  const openWizard = (mode: 'create' | 'edit' | 'view', budget?: Budget) => {
    setWizardMode(mode)
  ...
  const handleWizardSave = async (budgetData: Budget) => {
    try {
      if (wizardMode === 'create') {
        const response = await fetch('/api/gestor/budgets/dev', {
          method: 'POST',
      ...
      } else if (wizardMode === 'edit' && selectedBudget?.id) {
        const response = await fetch(`/api/gestor/budgets/${selectedBudget.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(budgetData),
        })
  ...
  const handleManagementUpdate = async (budgetData: Budget) => {
    try {
  ...
  // openEditModal removido — usar página de detalhe

  // calculateTotal removido
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={() => openWizard('create')} className="bg-blue-600 hover:bg-blue-700">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => openWizard('edit', budget)}
                      >
                    ...
                    <PopoverContent className="w-40">
                      <p className="text-sm">Editar orçamento</p>
                    </PopoverContent>
                  </Popover>
                )}
                {budget.status === 'draft' && (
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          toast.success(
                            `Replicação: ${j.created || 0} criados, ${j.existed || 0} já existiam`
                          )
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          // atualizar status e oferecer link
                          const s = await fetch(
                            `/api/gestor/orcamentos/${budget.id}/status`
                          )
                          const sj = await s.json().catch(() => ({}))
            ...
            <div className="lg:col-span-1 space-y-3">
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    const r = await fetch('/api/gestor/budgets/dev', {
                      method: 'POST',
                      headers: { 'content-type': 'application/json' },
                      body: JSON.stringify({
                        title: newTitle,
      ...
      {activeTab === 'catalog' && (
        <div className="mt-6">
          <CatalogToolbar query={catQuery} onQueryChange={setCatQuery} />
          <CatalogGrid
            data={catData}
            loading={catLoading}
            onProductSelect={id => {
