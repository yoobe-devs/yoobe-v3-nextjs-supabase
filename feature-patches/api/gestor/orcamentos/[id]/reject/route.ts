...
// POST - Rejeitar proposta do orçamento
export async function POST(
  ...
  try {
    ...
    if (orcamento.status !== 'proposal_sent') {
      return NextResponse.json(
        {
          error: 'Apenas orçamentos com proposta enviada podem ser rejeitados',
        },
        { status: 400 }
      )
    }
    ...
    // Atualizar status do orçamento para gestor_rejected
    const { error: updateOrcamentoError } = await supabaseService
      .from('budgets')
      .update({
        status: 'gestor_rejected',
        rejected_at: new Date().toISOString(),
        rejected_by: orcamento.manager_id,
        rejection_reason: reason || 'Rejeitado pelo gestor',
    ...
    console.error('Erro na API de rejeição de proposta:', error)
    return NextResponse.json(
