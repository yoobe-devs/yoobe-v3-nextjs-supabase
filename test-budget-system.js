...
async function testBudgetSystem() {
  ...
  try {
    ...
    let gestorUser
    try {
      const { data: newGestor, error: gestorError } = await supabase.auth.admin.createUser({
        email: gestorEmail,
        password: 'gestor123',
        email_confirm: true,
        user_metadata: {
    ...
    const { data: gestorSession, error: gestorLoginError } = await supabase.auth.signInWithPassword({
      email: gestorEmail,
      password: 'gestor123'
    })
    ...
    const budgetResponse = await fetch('http://localhost:3000/api/gestor/orcamentos', {
      method: 'POST',
      headers: {
    ...
    const adminBudgetsResponse = await fetch('http://localhost:3000/api/admin/orcamentos', {
      headers: {
    ...
    const approveResponse = await fetch(`http://localhost:3000/api/admin/orcamentos/${budgetResult.budget.id}/approve`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${adminToken}`
    ...
    const statusResponse = await fetch(`http://localhost:3000/api/clients/test-company-id/products/${replicateResult.product.id}/status`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${gestorToken}`
    ...
    const replicateWithoutBudgetResponse = await fetch('http://localhost:3000/api/gestor/base-products', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (replicateWithoutBudgetResponse.status === 403) {
      console.log('✅ Replicação bloqueada corretamente (sem orçamento aprovado)')
    } else {
