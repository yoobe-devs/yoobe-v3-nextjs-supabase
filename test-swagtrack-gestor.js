...
async function testSwagTrackGestor() {
  ...
  const pages = [
    ...
    {
      path: '/gestor/swag-track/orders/test-order-id',
      name: 'Página de Detalhes do Pedido',
    },
  ]
  ...
  for (const page of pages) {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        if (
          response.body.includes('Swag Track') ||
  ...
  console.log(
    '   1. Acesse a página principal: http://localhost:3001/gestor/swag-track'
  )
  ...
  console.log(
    '   - Detalhes do pedido: http://localhost:3001/gestor/swag-track/orders/[orderId]'
  )
  console.log(
    '   - APIs: /api/tracking/[orderId], /api/orders/search, /api/deliveries'
  )
}
