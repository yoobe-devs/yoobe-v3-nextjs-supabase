...
/**
 * Script para aplicar migrations do SwagTrack via container do PostgreSQL
 * Usa uma abordagem mais direta com o container do Supabase
 * Uso: node apply-migrations-container.js
 */

const { execSync } = require('child_process')

async function applyMigrationsContainer() {
  console.log('🚀 Aplicando migrations do SwagTrack via container...')
  console.log('')
...
`
    ...
    try {
      // Executar SQL via container
      execSync(`docker exec -i ${postgresContainer} psql -U postgres -d postgres -c "${migration1SQL}"`, { stdio: 'inherit' })
...
`
    ...
    try {
      // Executar SQL via container
      execSync(`docker exec -i ${postgresContainer} psql -U postgres -d postgres -c "${migration2SQL}"`, { stdio: 'inherit' })
