...
export default function GestorLayout({ children }: GestorLayoutProps) {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (session?.user) {
          ...
          setUser({
            name: session.user.user_metadata?.name || 'Gestor Yoobe',
            email: session.user.email,
            role: session.user.user_metadata?.role || 'manager',
            company_name: session.user.user_metadata?.company_name || 'Yoobe',
          })
          ...
          if (authError) {
            ...
            setUser({
              name: authUser.user_metadata?.name || 'Gestor Yoobe',
              email: authUser.email,
              role: authUser.user_metadata?.role || 'manager',
              company_name: authUser.user_metadata?.company_name || 'Yoobe',
            })
            setCompanyId(authUser.user_metadata?.company_id)
          } else {
  ...
  // Monta itens do sidebar com badges dinâmicas
  const baseItems = getGestorSidebarItems()
  const items = baseItems.map((it) => {
    if (it.title === 'Funcionários') {
      return { ...it, badge: countsLoading ? '…' : countsError ? '!' : (counts?.employees ?? it.badge) }
    }
    if (it.title === 'Produtos') {
      return { ...it, badge: countsLoading ? '…' : countsError ? '!' : (counts?.products ?? it.badge) }
    }
    if (it.title === 'Orçamentos') {
      return { ...it, badge: countsLoading ? '…' : countsError ? '!' : (counts?.quotations ?? it.badge) }
    }
    if (it.title === 'Pedidos') {
      return { ...it, badge: countsLoading ? '…' : countsError ? '!' : (counts?.orders ?? it.badge) }
    }
    return it
  })
