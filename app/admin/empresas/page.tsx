  ...
  const fetchCompanies = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('🏢 Número de empresas:', data.companies?.length || 0)
        
        // Adicionar campos calculados para compatibilidade
        const companiesWithCalculatedFields = data.companies?.map((company: Company) => ({
          ...company,
          stores_count: 0,
          users_count: 0,
          products_count: 0
        ...
        setCompanies(companiesWithCalculatedFields)
        toast.success(`Carregadas ${companiesWithCalculatedFields.length} empresas`)
      } else {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gerencie todas as empresas clientes do sistema</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-900">
              {companies.reduce((sum, company) => sum + (company.stores_count || 0), 0)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-900">
              {companies.reduce((sum, company) => sum + (company.users_count || 0), 0)}
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filtre as empresas por nome, email ou status</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {statuses.map(status => (
                  <option key={status} value={status}>
                    {status === 'all' ? 'Todos os Status' : 
                     status === 'active' ? 'Ativo' :
                     status === 'inactive' ? 'Inativo' : 'Pendente'}
                  </option>
                ))}
              </select>
