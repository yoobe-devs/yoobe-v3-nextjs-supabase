...
export async function PATCH(
  ...
  try {
    ...
    if (companyId !== requestedClientId) {
      return NextResponse.json({ error: 'Acesso negado - Você só pode gerenciar produtos da sua empresa' }, { status: 403 })
    }
    ...
    if (typeof is_active !== 'boolean') {
      return NextResponse.json({ error: 'Parâmetro is_active deve ser um boolean' }, { status: 400 })
    }
