...
async function ensureBudgetsSchema() {
  ...
  try {
    ...
    console.warn(
      'ensureBudgetsSchema: não foi possível executar exec_sql (pode não existir).',
      e?.message
    )
  }
}
  ...
  if (body.items) {
    body.items.forEach((item: any, index: number) => {
      ...
      if (
        ...
        errors.push(
          `Item ${index + 1}: Quantidade deve ser um inteiro positivo`
        )
      }
      if (
        item.custom_price &&
        ...
        errors.push(
          `Item ${index + 1}: Preço customizado deve ser um número não negativo`
        )
      }
      if (
        item.custom_points_cost &&
        (!Number.isInteger(item.custom_points_cost) ||
          item.custom_points_cost < 0)
      ) {
        errors.push(
          `Item ${index + 1}: Custo em pontos deve ser um inteiro não negativo`
        )
      }
    })
  }
  ...
  if (body.payment_methods && Array.isArray(body.payment_methods)) {
    body.payment_methods.forEach((method: any, index: number) => {
      ...
      if (
        ...
        (!Number.isInteger(method.points_amount) || method.points_amount < 0)
      ) {
        errors.push(
          `Método de pagamento ${index + 1}: Quantidade de pontos deve ser um inteiro não negativo`
        )
      }
      if (
        method.cash_amount &&
        (typeof method.cash_amount !== 'number' || method.cash_amount < 0)
      ) {
        errors.push(
          `Método de pagamento ${index + 1}: Valor em dinheiro deve ser um número não negativo`
        )
      }
    })
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query simplificada para orçamentos
    ...
    if (
      status &&
      Object.values(BUDGET_STATUSES).includes(status as BudgetStatus)
    ) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, items, addresses, payment_methods, sla_days } =
      body
    ...
    console.log('🔍 Dados que serão inseridos na tabela budgets:', budgetData)
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            details:
              budgetError.message || 'Falha na inserção no banco de dados',
          },
        },
        { status: 500 }
      )
    ...
    console.log('🔍 Itens que serão inseridos:', budgetItems)
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            details: itemsError.message || 'Falha na inserção dos itens',
          },
        },
        { status: 500 }
      )
