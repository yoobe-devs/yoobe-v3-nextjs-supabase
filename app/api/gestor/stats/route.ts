...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalPointsDistributed =
      pointsData?.reduce((sum, employee) => sum + employee.points_balance, 0) ||
      0
    ...
    try {
      const { data: inv } = await supabase
        ...
        .eq('client_id', companyId)
      if (inv?.length) {
        lowStockProducts = inv.filter(
          p => (p as any).stock_quantity <= 10 && (p as any).stock_quantity > 0
        ).length
        outOfStockProducts = inv.filter(
          p => (p as any).stock_quantity === 0
        ).length
        inventoryValue = inv.reduce(
          (sum, p) =>
            sum + ((p as any).price || 0) * ((p as any).stock_quantity || 0),
          0
        )
      }
    } catch {}
    ...
    const { data: recentRevenue } = await supabase
      ...
      .eq('status', 'delivered')
    const last30dRevenue =
      recentRevenue?.reduce((s, o) => s + (o.total_amount || 0), 0) || 0
