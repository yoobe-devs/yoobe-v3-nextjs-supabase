...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (add.length) await supabase.from('user_tags').insert(add.map(t => ({ user_id: params.id, tag_id: t })))
    if (remove.length) await supabase.from('user_tags').delete().in('tag_id', remove).eq('user_id', params.id)
