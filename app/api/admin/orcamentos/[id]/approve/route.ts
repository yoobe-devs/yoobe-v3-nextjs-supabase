...
export async function POST(
  ...
  try {
    ...
    if (userRole !== 'admin') {
      return NextResponse.json({ error: 'Acesso negado - Apenas administradores podem aprovar orçamentos' }, { status: 403 })
    }
    ...
    if (existingBudget.status !== 'pending') {
      return NextResponse.json({ error: 'Orçamento já foi revisado' }, { status: 400 })
    }
