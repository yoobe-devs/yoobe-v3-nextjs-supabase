...
async function createBudgetTables() {
  console.log('💰 Criando tabelas de orçamentos diretamente...\n')
  ...
  try {
    ...
    if (budgetError && budgetError.code === 'PGRST205') {
      ...
      // Primeiro, vamos verificar se conseguimos inserir dados
      ...
      if (insertError) {
        ...
        console.log('   📄 Arquivo: create-budget-tables.sql')
        return
      } else {
    ...
    console.log('💡 Se alguma tabela não foi criada, execute o SQL manualmente no Supabase Dashboard')
